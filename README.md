...
- Python3
- Python packags found in the requirements.txt (e.g. /opt/rocm-7.0.1/libexec/rocprofiler-compute/requirements.txt) of ROCm
...
and the AMD Version has been tested with ROCm 6.4.0 on the CDNA 2 architecture.
...
Generate two executables using the `nvcc` compiler with debug information:
- Executable generated by normal compilation, say `<executable_name>`. (`nvcc pr.cu -G -o pr`)
- Executable generated by using the `-cubin` flag with the `nvcc` compiler. If the executable name is prefixed with `cubin-`, i.e. `cubin-<executable_name>` (`nvcc pr.cu -G -cubin -o cubin-pr`), and is in the same folder as the regular executable, the cubin file path does not need to be entered later on.
...
```bash
./GPUscout -t nvidia -e ../executable/gaussian -a '-q -s 2000'
```
